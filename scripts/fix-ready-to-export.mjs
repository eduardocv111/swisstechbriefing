...
async function fix() {
    console.log("Fixing everything locally with llama3.1:8b...");
    ...
    for (const art of articles) {
        ...
        const ai = await askAI(`Extract 5 key facts from: ${trans.title}\n${trans.contentHtml.replace(/<[^>]*>/g, ' ').substring(0, 800)}`);
        if (ai) db.prepare("UPDATE Article SET keyFactsJson = ? WHERE id = ?").run(normalizeFacts(ai), art.id);
    }

    // Translations
    const trans = db.prepare("SELECT id, locale, title FROM ArticleTranslation").all();
    for (const t of trans) {
        console.log(`Fixing Translation: ${t.locale} - ${t.title}`);
        const ai = await askAI(`Extract 5 facts in ${t.locale} for: ${t.title}`);
        if (ai) db.prepare("UPDATE ArticleTranslation SET keyFactsJson = ? WHERE id = ?").run(normalizeFacts(ai), t.id);
    }
